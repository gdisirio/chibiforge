...
- `editable`: `"true"` or `"false"` — whether the field can be edited by the user.
- `visible`: `"true"` or `"false"` — whether the field is shown in the GUI.
- `default`: default value.

Optional dynamic attributes:

- **`editable_if`**: XPath expression evaluated against the live data model. When present, overrides `editable` — the field is editable only when the expression evaluates to true.
- **`visible_if`**: XPath expression evaluated against the live data model. When present, overrides `visible` — the field is shown only when the expression evaluates to true.
...
<property name="dma_channel_count"
          ...
          required="true"
          editable="true"
          editable_if="doc/dma_settings/use_dma = 'true'"
          visible="true"
          default="1"
          int_min="1"
          int_max="8"/>
...
<property name="pll_multiplier"
          ...
          editable="true"
          visible="true"
          visible_if="doc/initialization_settings/system_clock_source = 'PLL'"
          default="8"
          int_min="2"
          int_max="16"/>
```

The `*_if` attributes use XPath expressions that resolve against the same live data model used by `@ref:` (see §6.3). The GUI evaluates these expressions in real-time as the user edits values.
...
The generator builds a data model that is passed to FMPP as top-level variables. Each component's templates are processed with a data model scoped to that component. The same data model structure is used by the GUI for live `@ref:`, `visible_if`, and `editable_if` resolution.
...
4. **Live data model**:
   ...
   - `visible_if` and `editable_if` XPath expressions are evaluated in real-time as the user edits values, dynamically showing/hiding fields and toggling editability.
...
- Evaluate `visible` / `visible_if` to determine whether to show the field. If `visible_if` is present, it overrides `visible`.
- If visible, render the appropriate widget based on type (see §6.2).
- Evaluate `editable` / `editable_if` to determine whether the field is interactive. If `editable_if` is present, it overrides `editable`. Non-editable fields are rendered as read-only.
- Both `visible_if` and `editable_if` are re-evaluated whenever the data model changes (i.e., when the user edits any value), enabling reactive UI behavior.
...
- Reflect validation errors from:
  ...
  - `visible_if` / `editable_if` evaluation failures (invalid XPath).
...
- Any front‑end (JavaFX standalone app, Eclipse, VS Code extension via Webview, web app) can implement:
  ...
  - Live data model with real-time `@ref:`, `visible_if`, and `editable_if` resolution.
...
- **Maintain a live data model** for `@ref:`, `visible_if`, and `editable_if` resolution during editing.

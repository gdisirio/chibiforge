...
The output boundary constraint (must stay within configuration root) is a convention that component authors must follow. FMPP's `pp.changeOutputFile` operates relative to the output root, which is `generated/` for `cfg/` templates and the configuration root for `cfg_root_wa/`/`cfg_root_wo/` templates. There is no runtime enforcement of the boundary.
...
- If an ancestor section is effectively not visible or not editable, tools MAY skip evaluating `@cond:` expressions in that subtree.
...
# Addendum E – Implementation Decisions

This addendum records implementation decisions, clarifications, and scope adjustments made during development.

---

## E.1 GUI Scope Adjustments (Initial Release)

The following features from the UI specification are **deferred** in the initial release:

- **New Configuration flow**: the application opens existing `.xcfg` files only. The "New Configuration" option in the welcome screen and File menu is not implemented initially.
- **Files tab color-coding**: the Inspector Files tab shows a plain file tree of the configuration root. Cross-referencing with component containers to determine file provenance, color-coding (red/amber/grey), and badges (wa/wo/clean) are deferred to a later release.
- **Undo/Redo**: per-field scope only (each field tracks its own previous value), not a full command stack.

## E.2 GUI Behavioral Clarifications

- **Component ordering**: components appear in the Components view in creation order (the order they were added to the configuration). This matches the `<component>` element order in `chibiforge.xcfg`. Reorder buttons may be added in a future version.
- **Save with invalid fields**: save writes the DOM to disk. If a field contains invalid text, the DOM retains the last valid value (per §9.2 of the UI spec). The invalid text remains visible in the widget with an error indicator, but the saved `chibiforge.xcfg` reflects the last valid state.

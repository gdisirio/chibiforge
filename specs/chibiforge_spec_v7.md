...
<containerRoot>/
  component/
    ...
    cfg/                       # Templates (.ftl = classic, .ftlc = code-first)
      mcuconf.h.ftlc
      board.h.ftlc
...
- **Templates**: processed by FMPP with `generated/` as the default output directory.
...
### 8.2 Templates (`component/cfg/`)

Templates under `component/cfg/` are processed by **FMPP** (FreeMarker-based File PreProcessor). FMPP is configured with `<configurationRoot>/generated/` as the default output directory. Templates that do not redirect their output will produce files in `generated/` mirroring their relative path under `cfg/`.

Templates may use FMPP's output redirection APIs to write files to any location within the configuration root. The constraint enforced by ChibiForge is that any output path **must remain within the configuration root**.

#### 8.2.1 Template modes
...
#### 8.2.2 Default output mapping

By default (without explicit redirection), FMPP maps template paths to output paths by stripping the template extension:
...
Templates that need to write to the configuration root (e.g., linker scripts, Makefiles) or to specific subdirectories use FMPP's output redirection to place files at the desired location.
...
The generator builds a data model that is passed to FMPP as top-level variables. Each component's templates are processed with a data model scoped to that component. The same data model structure is used by the GUI for live `@ref:` resolution.
...
6. **Process each component**:
   - For each component in the configuration:
     ...
     d. **Process templates via FMPP**:
        - Configure FMPP with `generated/` as the default output directory and the component data model.
        - Process all templates under `component/cfg/`.
...
ComponentContent
  ...
  - list(prefix: String) : List<String>
    # e.g. list("cfg/"), list("source/"), list("source_root_wa/"), list("resources/")
```
...
9. For each component:
   - Load resources, build scoped data model, process static payload, process templates via FMPP.
...
For each `<property>` within a section or layout, render the appropriate widget based on type (see §6.2).
...
- Any front‑end (JavaFX standalone app, Eclipse, VS Code extension via Webview, web app) can implement:
  ...
  - Live data model with real-time `@ref:` resolution.
...
- **Maintain a live data model** for `@ref:` resolution during editing.
...
- Live data model enabling real-time constraint and default resolution.
...
- **`visible`**: returns `true`/`false` to conditionally show/hide the property based on other values.
- **`editable`**: dynamically controls whether the property is editable based on other values.
...
- Scripts should be minimal — a few lines at most — for simple conditional logic that cannot be expressed with static constraints.
...
## D.3 Conditional Visibility

Properties and sections could be conditionally shown or hidden in the GUI based on the values of other properties. This could be implemented via:

- The `visible` mini-script event (§D.1).
- A declarative `visible_if` attribute on properties/sections referencing other property values.
- Either mechanism would affect GUI rendering only — hidden properties retain their default values in the configuration.

---

## D.4 Dynamic Field Population
